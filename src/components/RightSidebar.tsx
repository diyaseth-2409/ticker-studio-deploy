    ...
    <aside className="flex w-[300px] shrink-0 flex-col border-l border-studio-border bg-white">
      {tickers.length > 1 && (
        <div className="border-b border-studio-border px-4 py-3">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-studio-ink-soft">Layers</h3>
          <LayersPanel />
        </div>
      )}
      ...
      {!ticker ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
          ...
          <p className="text-[12.5px] leading-relaxed text-studio-muted">
            Select a ticker on the canvas or from the left panel to edit its configuration.
          </p>
        </div>
      ) : (
